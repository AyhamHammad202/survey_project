  ...
  switch (raw.type) {
    ...
    case "open_world_text":
      return {
        ...
        placeholderAr: raw.placeholder
          ? getTextByLocale(raw.placeholder).ar
          : "اكتب إجابتك هنا...",
        placeholderEn: raw.placeholder
          ? getTextByLocale(raw.placeholder).en
          : "Type your answer...",
      };
...
export const COPY = {
  ...
  pressStart: {
    ar: introScene?.presentation?.action?.ar || "PRESS START",
    en: introScene?.presentation?.action?.en || "PRESS START",
  },
  ...
  validationError: {
    ar: "! اختر إجابة للمتابعة",
    en: "Please select an answer",
  },
  saving: { ar: "جاري الحفظ... 💾", en: "Saving... 💾" },
  demoBanner: {
    ar: "وضع تجريبي — أضف VITE_SHEETS_URL في .env",
    en: "Demo mode — set VITE_SHEETS_URL",
  },
};

export const QUESTIONS = (surveyData.questions || [])
  .map(normalizeQuestion)
  .filter((q) => q?.id && (q.textAr || q.textEn));
